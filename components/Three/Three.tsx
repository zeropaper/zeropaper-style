...
import { useRef, useEffect, PropsWithChildren, MutableRefObject, useCallback } from 'react';
...
export const RuntimeComponent = ({
  ...
  const handleRendererUnmount = useCallback((ctx: RendererCtx) => {
    ...
    contexts.current.splice(contexts.current.findIndex((c) => c.id === ctx.id), 1)
  ...
  useAnimationFrame((deltaTime) => {
    ...
    contexts.current.forEach(({
      ...
      try {
        ...
        destCtx?.drawImage(renderer.domElement, 0, 0, sw, sh, leftPrct * 0.01 * dw, topPrct * 0.01 * dh, sw, sh)
      } catch (e) {}
    })
  })
