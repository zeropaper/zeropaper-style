...
const ensureOtherCube = (scene: SceneCtx['scene'], recreate = false, name = 'other-cube') => {
  return ensure(name, (_THREE) => {
...
export const onMount = ({ scene, clock }: SceneCtx) => {
  ...
  cubes = new Array(w).fill(1).map((_, a) => new Array(h).fill(1).map((__, b) => new Array(d).fill(1).map((___, c) => {
    const cube = ensureOtherCube(scene, true, `clone-${a}-${b}-${c}`) as UnfoldingCube
